...
- **Floating Back-to-Top button** — A cyan, neon-glowing arrow that fades in only after you scroll down, sits independently above the chat launcher, and smooth-scrolls the page back to the top (hidden again at the top).
- **Scroll progress indicator** — A thin cyan bar pinned to the top of the viewport that fills with the page's scroll progress, and resets at the top.
...
- **Back-to-Top button** — Floating arrow that is hidden at the top, fades in after ~480px of scrolling, smooth-scrolls the page back to the top on click, and hides again at the top; positioned directly above the chat launcher with `aria-label` for accessibility
- **Scroll progress indicator** — Thin bar pinned at the top of the viewport that reflects current scroll progress (fills on scroll down, resets at the top)
- **Footer** — Branded logo, social icons (GitHub, LinkedIn, Hugging Face), an email link, and a Linktree link alongside the copyright notice
