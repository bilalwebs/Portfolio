...
export function Chatbot() {
  ...
  const { messages, sendMessage, status, error, setMessages } = useChat({
    ...
    messages: initial,
    transport,
    onError: (err) => console.error("[chat] request failed:", err),
  });
...
Ask me anything!`}
              ...
              {status === "error" && error && (
                <div className="flex gap-2">
                  ...
                  <div className="min-w-0 max-w-[85%] whitespace-pre-wrap rounded-2xl rounded-tl-sm border border-destructive/40 bg-destructive/10 px-3.5 py-2.5 text-sm leading-relaxed text-foreground">
